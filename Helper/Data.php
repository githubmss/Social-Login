...
{
    private $sociallogin_support = ['facebook', 'google'];
    ...
    private function sociallogin_support($sociallogintype)
    {
        if (!in_array($sociallogintype, $this->sociallogin_support)) :
            return ['status' => 'error', 'message' => __('Social Login is not supported by Magentomobileshop.')];
        else :
            return true;
        endif;
        return true;
    ...
    private function getFacebookdetails($token)
    {
        ...
        if (isset($response->email)) :
            ...
            return ['status' => 'error', 'message' => __('Token is invalid.')];
        endif;
        //@codingStandardsIgnoreEnd
    }
    ...
    private function checkuser($response)
    {
        try {
            ...
            $customer    = $_objectData->create('Magento\Customer\Model\Customer');
            ...
            if ($customer->getId()) :
                ...
                return ['status' => 'success', 'message' => $customerinfo];
            else :
                return $this->registerUser($response);
            endif;
        } catch (\exception $e) {
            return ['status' => 'error', 'message' => __($e->getMessage())];
        }
    }
    ...
    private function registerUser($response)
    {
        ...
        try {
            ...
            if ($customer->isConfirmationRequired()) :
                $customer->sendNewAccountEmail('confirmation', $session
                ->getBeforeAuthUrl(), $this->_storeManager->getStore()->getId());
                return ['status' => 'error', 'message' => __('Account confirmation required.')];
            else :
                $session->setCustomerAsLoggedIn($customer);
                $customer->sendNewAccountEmail('registered', '', $this->_storeManager->getStore()->getId());
            endif;
            return ['status' => 'success', 'message' => $this->checkStatus()];
        } catch (\Exception $ex) {
            return ['status' => 'error', 'message' => __('Error in creating user account.')];
        }
    }
